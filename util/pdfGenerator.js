...
function generateFacture(body){
  ...
  const qrCode = readFileSync(path.join(__dirname,"qr_code.png"));

    const doc = new jsPDF();
    const head = [["Nom Emetteur","Prenom Emetteur","Téléphone Emetteur","Montant (DH)","Date d'Opération"]];
      ...
      didDrawPage: (data) => {
        ...
        if(pageNumber==1){
            ...
            doc.text('IF: '+body.destinataire.marchandData.IF, data.settings.margin.left*3, 50);
              ...
              doc.text(`Facture N°: ${body.destinataire.marchandData.nomMarchand} - ${body._id.toString().substring(0,5)}`, doc.internal.pageSize.width-data.settings.margin.right, 67,{align:'right'});
